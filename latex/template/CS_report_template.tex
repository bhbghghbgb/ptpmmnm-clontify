%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%----------------------------------------------------------------------------------
% DO NOT Change this is the required setting A4 page, 11pt, onside print, book style
%%----------------------------------------------------------------------------------
\documentclass[a4paper,11pt,oneside]{book} 
\usepackage{CS_report} % DO NOT REMOVE THIS LINE. 

\usepackage[vietnamese]{babel}
\usepackage{comment}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\def\groupno{8}
\def\groupuninamel{Trường Đại Học Sài Gòn}
\def\groupuniname{SGU}
\def\groupdepnamel{Công Nghệ Thông Tin}
\def\groupdepname{CNTT}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{document}

    \captionsetup[figure]{margin=1.5cm,font=small,name={Figure},labelsep=colon}
    \captionsetup[table]{margin=1.5cm,font=small,name={Table},labelsep=colon}
    
    \frontmatter
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    \begin{titlepage}      
        \begin{center}
            \includegraphics[width=3cm]{figures/uorlogo.png}\\[0.5cm]
            % {\LARGE University of Reading\\[0.5cm]
            % Department of Computer Science}\\[2cm]
            {\LARGE \groupuninamel\\[0.5cm]
            Khoa \groupdepnamel}\\[2cm]
			%{\color{blue} \rule{\textwidth}{1pt}}
			
			% -------------------------------
			% You need to edit some details here
			% -------------------------------  
            \linespread{1.2}\huge {
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%
                %TODO: 1 TITLE of Your PROJECT 
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%
                % change the following line                
                % Computer Science Report Template and Report Writing Guide
                Xây dựng ứng dụng Clone Spotify
            
            }
            \linespread{1}~\\[2cm]
			%{\color{blue} \rule{\textwidth}{1pt}}
            {\Large 
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%
                %TODO: 2 YOUR NAME
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%             
                % change the following line
                % FirstName(s) LastName
                3121410071 - Huỳnh Gia Bảo - huynhgiabaodhsg@gmail.com
                3121410071 - Huỳnh Gia Bảo - huynhgiabaodhsg@gmail.com
                3121410071 - Huỳnh Gia Bảo - huynhgiabaodhsg@gmail.com
                % change end             
            }\\[1cm] 
            

            {\large 
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%
                %TODO: 3 YOUR NAME Supervisor's name(s)
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%             
                % change the following line                
                % \emph{Supervisor:} Supervisor's Name}\\[1cm] % if applicable
                \emph{Giảng viên:} Từ Lãng Phiêu}\\[1cm]

\begin{comment}
    		% PLEASE DO NOT CHANGE THIS TEXT %
            \large A report submitted in partial fulfilment of the requirements of\\the University of Reading for the degree of\\
            %%%
            %TODO:  verify your degree name
            Master of Science 
            %% 
            %TODO:  verify your degree in
            in \textit{Data Science and Advanced Computing}\\[0.3cm] 
\end{comment}
            \vfill
            
            
            \today % Please update this date you can use \date{April 2020} for fixed date
        \end{center}
    \end{titlepage}
    
    
\begin{comment}
    % -------------------------------------------------------------------
    % Declaration
    % -------------------------------------------------------------------
    \newpage
    \thispagestyle{empty}
    % \chapter*{\Large Declaration}
    \chapter*{\Large Lời cam đoanđoan}
    % PLEASE CHANGE THIS TEXT EXCEPT YOUR NAME%
    % -------------------------------
    %TODO: PLEASE ONLY UPDATE HERE -- PLEASE WRITE YOUR NAME %
    % -------------------------------
    I,
    %%%%%%%%%%%%%%%%%%%%%%%
    Firstname(s) Lastname, % Madatory part  TODO
    %%%%%%%%%%%%%%%%%%%%%%%
    of the Department of Computer Science, University of Reading, confirm that this is my own work and figures, tables, equations, code snippets, artworks, and illustrations in this report are original and have not been taken from any other person's work, except where the works of others have been explicitly acknowledged, quoted, and referenced. I understand that if failing to do so will be considered a case of plagiarism. Plagiarism is a form of academic misconduct and will be penalised accordingly. \\

    %% Please delete as appropriate.
    \noindent
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %TODO 1 Consent for example copy -  we will use
    I give consent to a copy of my report being shared with future students as an exemplar. \\

    \noindent
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %TODO 2 Consent to let the report to use use by library for public use
    I give consent for my work to be made available more widely to members of UoR and public with interest in teaching, learning and research.
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ~\\[1cm]
    \begin{flushright}
    %------------------------------
    % change the following line
    %TODO: PLEASE UPDATE  Your Name  -------------------------------%
    Firstname(s) Lastname % Please change it to your name

    \today
    \end{flushright}
\end{comment}
    % -------------------------------------------------------------------
    % Lời cam đoan (phiên bản tiếng Việt đã chỉnh sửa)
    % -------------------------------------------------------------------
    \newpage
    \thispagestyle{empty}
    \chapter*{\Large Lời cam đoan}
    Chúng em, Nhóm \groupno, xác nhận rằng đây là công trình của riêng nhóm và các hình ảnh, bảng biểu, phương trình, đoạn mã, tác phẩm nghệ thuật và hình minh họa trong báo cáo này là nguyên bản và không được lấy từ công trình của bất kỳ người nào khác, ngoại trừ những trường hợp mà các công trình của người khác đã được trích dẫn, ghi nhận và tham khảo rõ ràng. Chúng em hiểu rằng việc không tuân thủ điều này sẽ bị coi là hành vi đạo văn. Đạo văn là một hình thức vi phạm học thuật và sẽ bị xử lý theo quy định.\\
    
    \noindent
    Chúng em đồng ý cho phép chia sẻ bản sao báo cáo này với sinh viên các khóa sau như một tài liệu tham khảo.\\
    
    \noindent
    Chúng em đồng ý cho phép thư viện sử dụng báo cáo này rộng rãi hơn cho các thành viên của trường và công chúng quan tâm đến giảng dạy, học tập và nghiên cứu.\\
    ~\\[1cm]
    \begin{flushright}
    	Nhóm \groupno
    
    	\today
    \end{flushright}


\begin{comment}
    % -------------------------------------------------------------------
    % Abstract and Acknowledgement
    % -------------------------------------------------------------------
    
    \input{chapters/00_i_abstract}
    % -------------------------------------------------------------------
	% Acknowledgement
	% -------------------------------------------------------------------
   
    \input{chapters/00_ii_acknowledgments}   
\end{comment}

    % -------------------------------------------------------------------
    % Contents, list of figures, list of tables
    % -------------------------------------------------------------------
    
    \tableofcontents
    \listoffigures
    \listoftables
    \input{glossaries} %  Enter your list of Abbreviation and Symbols in this file



    
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%                                                                    %%  
    %%  Main chapters and sections of your project                        %%  
    %%  Everything from here on needs updates in your own words and works %%
    %%                                                                    %%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    \mainmatter
    % Read for preparation of document in LaTex 
    % Lamport, L. (1986), LATEX: A Document Preparation System, Addison-Wesley.
    
    % \input{chapters/01_introduction}
    \input{my_chapters/01_introduction}
    % \input{chapters/02_literature} % https://guides.library.bloomu.edu/litreview
    \input{my_chapters/02_techstacks}
    % \input{chapters/03_methodology}
    \input{my_chapters/03_projectstructure}
    % \input{chapters/04_results}
    \input{my_chapters/04_result}
    % \input{chapters/05_discussion}
    \input{my_chapters/05_installation}
    % \input{chapters/06_conclusions}
    \input{my_chapters/06_completions}
    % \input{chapters/07_reflection}


    

    
    % -------------------------------------------------------------------
    % Bibliography/References  -  Harvard Style was used in this report
    % -------------------------------------------------------------------

\begin{comment}
    Bạn chỉ cần điền nội dung chi tiết vào các phần hướng dẫn cài đặt và thông tin về phân công công việc. File \texttt{references.bib} đã chứa các tài liệu tham khảo. Khi biên dịch LaTeX, lệnh \texttt{\textbackslash bibliography{references}} sẽ tự động tạo danh sách tài liệu tham khảo dựa trên file này và \texttt{\textbackslash bibliographystyle{unsrt}} sẽ quy định cách hiển thị các mục. Lệnh \texttt{\textbackslash nocite{*}} đảm bảo tất cả các mục trong \texttt{references.bib} đều được hiển thị trong danh sách tài liệu tham khảo, ngay cả khi chúng không được cite trực tiếp trong báo cáo.
\end{comment}
    \nocite{*}

    \bibliography{references}  %  Patashnik, O. (1988), BibTEXing. Documentation for general BibTEX users.
    
    % -------------------------------------------------------------------
    % Appendices
    % -------------------------------------------------------------------
\begin{comment}
    \begin{appendices}
        \input{chapters/appendix_A.tex}
        \input{chapters/appendix_B.tex}
    \end{appendices}
\end{comment}
    
\end{document}
